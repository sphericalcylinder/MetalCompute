...
template <typename T> class GPU {
  ...
  private:
    ...
    MTLCompute::CommandManager<T>
        commandManager; ///< The MetalCompute command manager object
  ...
  public:
     ...
     * and creates a new GPU object, MTLCompute::Kernel object, and
     * MTLCompute::CommandManager object
     *
    ...
    GPU(std::string libname, std::string functionName)
        ...
        this->commandManager =
            MTLCompute::CommandManager<T>(this->gpu, &this->kernel);
     ...
     *
     * Creates a new GPU object, MTLCompute::Kernel object, and
     * MTLCompute::CommandManager object
     *
     */
     ...
     *
     */
    ~GPU() { this->gpu->autorelease(); }

    /**
     * @brief Load a metal library and function
     *
     * Takes in the name of the Metal library and the name of the function
     * and creates a new MTLCompute::Kernel object and
     * MTLCompute::CommandManager object
     *
     * @param libname The name of the Metal library
     * @param functionName The name of the function
    ...
    void loadArray(vec<T> &array, int index) {
        this->checkloaded();
        MTLCompute::Buffer<T> buffer(gpu, array.size(),
     ...
     *
     * Takes in a vector of vectors of type T and an index and loads the matrix
     * into the GPU at that index
     *
     ...
     *
     * Takes in a vector of vectors of vectors of type T and an index and loads
     * the matrix into the GPU at that index
     *
     * @param matrix The matrix to load
     * @param index The index to load the matrix into
     *
    ...
    void loadMatrix(vec3<T> &matrix, int index) {
        this->checkloaded();
        MTLCompute::Texture3D<T> texture(gpu, matrix[0][0].size(),
