...
template <MTLCompute::ResourceStorage StorageMode> void BufferConversionTests() {

    SUBCASE("Test Buffer to Texture1D") {
    ...
    SUBCASE("Test oversize Buffer to Texture1D") {
        MTLCompute::Buffer<int> buffer(gpu, MTLCompute::MAX_TEXTURE1D_SIZE + 1, StorageMode);
    ...
    SUBCASE("Test wrong type Buffer to Texture1D") {
        struct imaginary_type { int a = 1; };
        MTLCompute::Buffer<imaginary_type> buffer(gpu, 10, StorageMode);
    ...
    SUBCASE("Test Texture1D to Buffer") {
        MTLCompute::Texture1D<int> texture(gpu, 10);
        ...
        MTLCompute::Buffer<int> buffer2(gpu, 10, StorageMode);
    ...
    SUBCASE("Test TextureBuffer to Buffer") {
    ...
    SUBCASE("Test Buffer to TextureBuffer") {
        MTLCompute::Buffer<int> buffer(gpu, 10, StorageMode);
        REQUIRE_NOTHROW(buffer = vec<int>(10, 1));
    ...
    SUBCASE("Test oversize Buffer to TextureBuffer") {
        MTLCompute::Buffer<int> buffer(gpu, MTLCompute::MAX_TEXTUREBUFFER_SIZE + 1, StorageMode);
    ...
    SUBCASE("Test wrong type Buffer to TextureBuffer") {
        struct imaginary_type { int a = 1; };
        MTLCompute::Buffer<imaginary_type> buffer(gpu, 10, StorageMode);
        buffer = vec<imaginary_type>(10, imaginary_type());
...
TEST_CASE("Test Shared Memory Buffer Conversions") {
    BufferConversionTests<MTLCompute::ResourceStorage::Shared>();
}
TEST_CASE("Test Managed Memory Buffer Conversions") {
    BufferConversionTests<MTLCompute::ResourceStorage::Managed>();
}
